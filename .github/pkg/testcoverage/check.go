...
func Check(w io.Writer, cfg Config) bool {
	currentStats, err := GenerateCoverageStats(w, cfg)
	if err != nil {
...
func GenerateCoverageStats(w io.Writer, cfg Config) ([]coverage.Stats, error) {
	return coverage.GenerateCoverageStats(w, cfg.ReportUncovered, coverage.Config{ //nolint:wrapcheck // err wrapped above
