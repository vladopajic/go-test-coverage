...
func Test_GenerateCoverageStats(t *testing.T) {
	...
	stats, err := GenerateCoverageStats(w, false, Config{Profiles: []string{t.TempDir()}})
	assert.Error(t, err)
	assert.Empty(t, stats)

	// should get error parsing invalid profile file
	stats, err = GenerateCoverageStats(w, false, Config{Profiles: []string{profileNOK}})
	assert.Error(t, err)
	assert.Empty(t, stats)

	// should be okay to read valid profile
	stats1, err := GenerateCoverageStats(w, false, Config{Profiles: []string{profileOK}})
	assert.NoError(t, err)
	assert.NotEmpty(t, stats1)

	// should be okay to read valid profile
	stats2, err := GenerateCoverageStats(w, false, Config{
	...
	stats3, err := GenerateCoverageStats(w, false, Config{
		Profiles:    []string{profileOK},
		LocalPrefix: prefix,
	})
	assert.NoError(t, err)
	...
	stats4, err := GenerateCoverageStats(w, false, Config{
	...
	stats5, err := GenerateCoverageStats(w, false, Config{
...
func Test_coverageForFile(t *testing.T) {
	...
	s = CoverageForFile(w, false, profile, funcs, blocks, annotations)
